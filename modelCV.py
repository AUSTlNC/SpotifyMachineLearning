...
def holdout(model, xFeat, y, testSize):
    ...
    trainPred = model.predict_proba(xTrain)
    testPred = model.predict_proba(xTest)
...
def kfold_cv(model, xFeat, y, k):
    ...
    for train_index, test_index in kFold.split(xFeat, y):
...
def mc_cv(model, xFeat, y, testSize, s):
    ...
    mc.split(xFeat, y)
    xTrain, xTest, yTrain, yTest = None, None, None, None
    for train_index, test_index in mc.split(xFeat, y):
        xTrain, xTest = xFeat.iloc[train_index], xFeat.iloc[test_index]
        yTrain, yTest = y.iloc[train_index], y.iloc[test_index]
    model = model.fit(xTrain, yTrain)
    trainPred = model.predict_proba(xTrain)
